...
import (
	"flag"
	"fmt"
	doc "github.com/a-urazov/vm/docs"
...
func genDocs(path string, cfg doc.Config, isDir bool) {
	...
	for _, d := range list {
		if strings.HasSuffix(d.Name(), ".bs") {
...
func _main_mdoc() {
	flag.Usage = func() {
		fmt.Fprintf(os.Stderr, "Usage: %s [billing file]\n", os.Args[0])
		flag.PrintDefaults()
		os.Exit(0)
	}
