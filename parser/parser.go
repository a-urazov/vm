...
import (
	"bytes"
	"fmt"
	"github.com/a-urazov/vm/ast"
	"github.com/a-urazov/vm/lexer"
	"github.com/a-urazov/vm/token"
	"io/ioutil"
	"os"
	"path/filepath"
	"sort"
...
func (p *Parser) ParseProgram() *ast.Program {
	...
	//if the billing file only have ';', then we should return earlier.
	...
	for p.curToken.Type != token.EOF {
		...
		if stmt != nil {
			if i, ok := stmt.(*ast.IncludeStatement); ok {
				importPath := strings.TrimSpace(i.IncludePath.String())
				_, ok := program.Includes[importPath]
				if !ok {
					program.Includes[importPath] = i
				}
			} else {
...
func (p *Parser) parseIncludeStatement() *ast.IncludeStatement {
	...
	importPath := strings.TrimSpace(stmt.IncludePath.String())
	...
	program, err := p.getIncludedStatements(importPath)
	if err != nil {
...
func (p *Parser) getIncludedStatements(importpath string) (*ast.Program, error) {
	...
	fn := filepath.Join(path, importpath+".bs")
	f, err := ioutil.ReadFile(fn)
	if err != nil { //error occurred, maybe the file do not exists.
		// Check for 'BILLING_ROOT' environment variable
		importRoot := os.Getenv("BILLING_ROOT")
		if len(importRoot) == 0 { //'BILLING_ROOT' environment variable is not set
			return nil, fmt.Errorf("Syntax Error:%v- no file or directory: %s.bs, %s", p.curToken.Pos, importpath, path)
		} else {
			fn = filepath.Join(importRoot, importpath+".bs")
			e, err := ioutil.ReadFile(fn)
			if err != nil {
				return nil, fmt.Errorf("Syntax Error:%v- no file or directory: %s.bs, %s", p.curToken.Pos, importpath, importRoot)
			}
			f = e
		}
	}
