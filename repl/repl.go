...
const PROMPT = "billing>> "

func Start(out io.Writer, color bool) {
	history := filepath.Join(os.TempDir(), ".billing_history")
	l := liner.NewLiner()
	defer l.Close()
	...
	if color {
		...
		//l.RegisterKeywords(billingKeywords)
		//l.RegisterOperators(billingOperators)
