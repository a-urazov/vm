...
import (
	...
	_ "github.com/a-urazov/vm/lexer"
	...
	defer func() {
		if r := recover(); r != nil {
			...
			case runtime.Error:
				...
				fmt.Println(r.Error(), fmt.Errorf(msgFmt, filename, lineNum, funcName, r, prettyStack))
				//panic(r)
				val = NIL
				return
			}
...
func loadIncludes(imports map[string]*ast.IncludeStatement, scope *Scope) {
...
func evalIncludeStatement(i *ast.IncludeStatement, scope *Scope) Object {
	...
	if _, ok := importScope.Get(i.IncludePath.String()); !ok {
		evalProgram(i.Program, imported.Scope)
		importScope.Set(i.IncludePath.String(), imported)
	}
...
func evalInfixExpression(node *ast.InfixExpression, left, right Object, scope *Scope) Object {
	...
	if _, ok := left.(Writable); ok { //There are two Writeables in billing: FileObject, HttpResponseWriter.
...
// Eval when a function is _called_, imports fn literal evaluation and calling builtins
...
func evalClassLiteral(c *ast.ClassLiteral, scope *Scope) Object {
	var parentClass = BASE_CLASS //base class is the root of all classes in billing
	if c.Parent != "" {
