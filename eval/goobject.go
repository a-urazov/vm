...
func (gfn *GoFuncObject) CallMethod(line string, scope *Scope, method string, args ...Object) Object {
	...
	// Convert the result back to billing Object
...
// Billing language Object to go language Value.
...
// Go language Value to billing language Object(take care of slice object value)
...
func RegisterVars(name string, vars map[string]interface{}) {
	for k, v := range vars {
		...
		SetGlobalObj("vm."+name+"."+k, GoValueToObject(v))
		SetGlobalObj("go."+name+"."+k, NewGoObject(v))
	}
}
