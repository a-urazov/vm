...
type Scope struct {
	...
	//We need to use `Mutex`, because we added 'thread'(multithread).
	//if not，when running `thread`, there will be lot of errors, even core dump.
...
func (s *Scope) Set(name string, val Object) Object {
	...
	if ok && s.readonly[name] {
		log.Printf("[VM] Const variable '%s' cannot be modified.\n", name)
		return val
	}
