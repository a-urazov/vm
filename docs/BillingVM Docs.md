# Billing Programming Language

Table of Contents
=================
<details><summary>TOC</summary>

- [Billing Programming Language](#billing-programming-language)
...
Billing is a toy language interpreter, written in Go. It has C-style syntax, and is largely inspired by Ruby, Python, Perl and c#
...
This is a sample program using billing language:
...
```sh
~ » billing
Billing programming language REPL
...
Billing support two kinds of single line comment and also block comment.
...
Billing supports 9 basic data types: `String`, `Int`, `UInt`, `Float`, `Bool`, `Array`, `Hash`, `Tuple` and `Nil`
...
In billing, there are mainly eleven types of constants(Literals).
...
Variables in Billing could start with the keyword `let`, or nothing with the
form `variable=value`.
...
Note, the left-hand side must be importd using the '()'.
...
Note：Starting from Billing 5.0，when the decalared variable already exists, it's value will be overwritten:
...
In Billing, you can use enum to define constants.
...
### Meta-Operators
Billing has some meta-operators borrowed from perl6.
...
At the moment, Billing has six meta-operators：
* <p>~+</p>
* <p>~-</p>
* <p>~*</p>
* <p>~/</p>
...
```
### using statement
In billing, if you have some resources you want to release/free/close, e.g. close opened file, close network connection etc，
...
In billing, you are free to define some operators, but you cannot
...
In billing, integer is treated as an object, so you could call it's methods.
...
In billing, float is also treated as an object, so you could call it's methods.
Please see below examples:
...
In billing, decimal is Arbitrary-precision fixed-point decimal numbers.
...
In billing, you could use [] to initialize an empty array:
...
In billing, there are three types of `string`:
...
In billing, strings are utf8-encoded, you could use utf-8 encoded name as a variable name.
...
### Hash
In billing, the builtin hash will keep the order of keys when they are added to the hash, just like python's orderedDict.
...
In billing, Hash is also an object, so you could use them to operate on hash object:
...
In billing, `tuple` is just like array, but it could not be changed once it has been created.
...
### Extend basic type 
Billing also provides support for extending basic types.
...
### Optional type
Billing has support for Optional type like java8.
...
Billing has limited support for the oop concept, below is a list of features:
...
The billing parser could parse `public`, `private`, `protected`, but it has no effect in the evaluation phase.
That means billing do not support access modifiers at present.
...
In billing, all class is inherited from the root class `object`. 
`object` class import some common method like `toString()`, `instanceOf()`, `is_a()`, `classOf()`, `hashCode`.
...
Billing has support for class `indexer`(like c#). 
...
Billing also support class Category like objective-c（C# is called 'extension methods'）.
...
Billing also has very simple annotation support like java：
...
Billing also import some builtin annotations:
...
Maybe you are curious about why `nil` or `false` have message() function? Because in billing, `nil` and `false`
...
In billing, you could concatenate of different types. See below for examples:
...
Billing support list(array,string, range, tuple) comprehensions.
...
Billing also support hash comprehension.
...
Function in billing is a first-class object. This means the language supports passing functions as arguments to
...
Before ver5.0, Billing do not support multiple return values, But there are many ways to do it.
...
Starting from ver5.0, Billing support multiple return values using 'let'.
...
You can use `thread` to create a new thread, and `chan` to communicate with the thread.
...
```
You could use channel and thread togeter to support lazy evaluation:
...
Billing has experimental support for working with `go` modules.

If you need to use go`s language package function, you first need to use `RegisterFunctions` or `RegisterVars` to
register `go` language functions or types into billing language.

Below is an example of `main.go`(extracted):

```swift
// Because in billing we already have built in module `fmt`, here we use `gfmt` for package name.
...
```
Now, in your billing file, you could use it like below:
...
For more detailed examples, please see `goObj.bs`.

## Standard module introduction

In billing, there are some standard modules provided for you. e.g. json, sql, sort, fmt, os, logger, time, flag, net, http, etc...
This is a brief introduction of some of the billing standard modules, don't expect it to be thorough.
...
In billing, the `linq` module support seven types of object:
...
Now, billing has a powerful `linq for file` support. it can be used to operate
...
3. Recompile billing source.

Below is a complete source of the `examples/db.bs`:
...
In billing, regard to regular expression, you could use:
...
The formatter utility can format the billing language.
The highlighter utility can highlight the billing language to console or html.

You could also combine the two utilities:

```sh
./fmt xx.bs | ./highlight  //output to console(console highlight not support windows)
```
...
./mdoc -html -showsource -css 1 examples/doc.bs
...
./mdoc -html -showsource -css 1 -cssfile ./examples/github-markdown.css examples/doc.bs

//processing all the '.bs' files in examples directory, generate html.
./mdoc -html examples
```
